...
1. `Maybe`: Encapsulates the possible absense of value. An optional value. There are some `Some` or `None`;
2. `Either`: Encapsulates a value of one of two possible types, `Left` or `Right`.
3. `Result`: Encapsulates a `Success` value or a `Failure` (error).
